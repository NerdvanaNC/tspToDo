...
<main class="w-96 h-[45vh]">
  <div class="card w-full h-full m-0 bg-base-200">
    <h2 class="card-title bg-primary px-8 py-4 text-primary-content rounded-tl-md rounded-tr-md">Register</h2>
    <div class="py-10 px-8 overflow-auto text-md">
      <form action="?/register" method="POST" class="form-control" use:enhance>
        <input type="text" name="username" placeholder="Username:" class="input w-full mb-6" />
        <input type="password" name="password" placeholder="Password (8+ characters):" class="input w-full mb-6" />
        <input type="password" name="passwordConfirm" placeholder="Password (again):" class="input w-full mb-12" />
